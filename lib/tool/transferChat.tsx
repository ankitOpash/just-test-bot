...
export async function isHumanChatRequest(message: string): Promise<boolean> {
  const lowerCaseMessage = message.trim().toLowerCase().split(/\s+/).join(" ");

  const openai = new ChatOpenAI({
    openAIApiKey: process.env.OPENAI_API_KEY,
    modelName: "gpt-4o-mini", // Or gpt-3.5-turbo, gpt-4-0314, or your preferred model.  "gpt-4o-mini" is not a standard model name.
  ...
  try {
    ...
    if (trimmedResponse === "true") {
      ...
      console.error("Unexpected response from OpenAI:", trimmedResponse); // Log unexpected responses for debugging
      return false; // Default to false if the response is unclear.
    }
  } catch (error) {
    console.error("Error invoking OpenAI:", error); // Log errors for debugging
